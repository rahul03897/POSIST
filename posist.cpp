...
void getData(int head,vector<struct ChildNode*> headChildNodes)
{

    int size = headChildNodes.size();
    string name,pass,algo="qwerty";
    float value;

    struct ChildNode *ChildNode1 = new ChildNode();
    struct GNode *data1=new GNode();
    ...
    {
        ChildNode1->ref=NULL;
        headChildNodes.push_back(ChildNode1);
    }
    else
    {
...
int main()
{
    vector<struct ChildNode *> headChildNodes;
    ...
    while(1)
    {
        ...
        if(num==7)
        {
            ...
            struct ChildNode* newowner=all[nNum2];
        ...
        if(num==8)
        {
            ...
            for(int i=0; i<headChildNodes.size(); i++)
            	longest(headChildNodes[i],ma);
